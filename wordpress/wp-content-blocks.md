...
It accepts a list of `Data` items that contain arbitrary objects. Each need to contain two properties for the unique `id` and `parentId` fields. The `parentId` should point to an `id` of the parent `Data` item.
